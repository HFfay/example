...
/**
 * 微服务使用feign相互之间调用，当引入Hystrix熔断策略时，Hystrix默认隔离策略是THREAD，是没办法拿到 ThreadLocal 中的Request
 * 虽然可以改成SEMAPHORE策略， 但是Hystrix官方强烈建议使用THREAD作为隔离策略
 *
 * 参数Spring Security和Sleuth的自定义并发策略
 */
